...
- **文件管理**：按用户浏览文件树、上传、批量上传、新建目录、下载；目录内支持**面包屑导航 + 返回上一级 + 双击目录行进入**，文件行支持**双击预览**，新建目录后停留当前目录、可打开或返回；常见文件（图片、音视频、文本/代码、PDF）支持**在线预览**，无需下载即可查看（文本/代码与 PDF 通过浏览器内嵌渲染，`text/html` 强制下载以防 XSS）；下载带实时进度条与速度显示；删除文件/目录经**自定义确认弹窗**（危险操作风格：脉冲图标 + 红色警示 + 明确「不可恢复」提示）二次确认
...
> 界面采用「深空控制台风」设计（玻璃拟态 + 青色辉光 + 等宽技术字体 + 入场动画），样式与脚本位于 `web/assets/`。Google Fonts 异步加载并配系统字体强回退，无网时自动降级为系统字体，保证内网/离线环境仍可正常使用。前端登录/注册输入框提供聚焦提示、格式示例与动态校验反馈；删除类危险操作使用**自定义玻璃拟态确认弹窗**（替换浏览器原生 `confirm()`），带脉冲图标、红色警示与快捷键（Esc 取消 / Enter 确认），交互与整体风格完全统一。
