...
- **文件管理**：按用户浏览文件树、上传、批量上传、新建目录、下载；目录内支持**面包屑导航 + 返回上一级 + 双击目录行进入**，文件行支持**双击预览**，新建目录后停留当前目录、可打开或返回；新建目录经**自定义输入弹窗**（青色图标 + 居中输入框 + 实时名称校验，禁用 `/\:*?"<>|` 等非法字符）；常见文件（图片、音视频、文本/代码、PDF）支持**在线预览**，无需下载即可查看（文本/代码与 PDF 通过浏览器内嵌渲染，`text/html` 强制下载以防 XSS）；预览仅内联展示、**绝不触发下载**，下载需通过明确的「下载」按钮触发放置；对 Office 等**无法内网预览**的格式（doc/docx/xls/xlsx/ppt/pptx 等）预览弹窗给出**明确下载引导**（提示「请下载后使用 Office / WPS 打开」+「⬇ 下载文件」按钮）；下载带实时进度条与速度显示；**批量删除 + 回收站**：文件列表带**勾选列**，工具栏支持**全选 / 反选 / 取消选择**，批量操作栏显示已选数并提供「删除选中」按钮，删除前经**自定义确认弹窗**二次确认（含选中项数量与名称预览）；删除操作（单删/批量）统一**移入回收站**而非物理删除，回收站视图支持**恢复**与**清空**；单删成功 toast 带**「撤销」按钮**一键恢复；后端 `DELETE /api/files` 与 `POST /api/files/batch-delete` 均经**权限校验**（仅可写用户），防护越权与主目录根路径误删，`GET /api/trash` / `POST /api/trash/restore` / `DELETE /api/trash` 管理回收站；**上传同名冲突处理**：上传前自动检测目标目录是否已有同名文件，若有则弹出**冲突确认弹窗**（黄色警示列出冲突文件），提供「覆盖原文件 / 自动重命名 / 取消上传」三种处理方式——覆盖直接替换、自动重命名在文件名后追加时间戳序号（如 `file_123456.txt`）、取消则不写入；后端 `POST /api/upload` 支持 `mode` 参数（`rename`/`overwrite`/`cancel`），`cancel` 模式遇冲突返回 409 + 冲突清单；**前端分页 / 排序 / 筛选**：文件列表支持**前端分页**（数据来自 `GET /api/files` 整目录返回，客户端切片，零额外请求），默认每页 **50** 条、可切换 **20 / 50 / 100 / 全部**；分页导航含「上一页 / 下一页 + 页码（带省略号）+ 当前/总页」，并实时显示「显示 X–Y / 共 N 项（第 a/b 页）」；表头「名称 / 大小 / 修改时间」可点击**排序**（目录优先，升/降切换，表头显示 ▲▼ 指示）；工具栏提供「全部 / 文件夹 / 文件」**筛选芯片**；分页、排序、筛选三者叠加（筛选 → 排序 → 分页切片）并与**搜索**联动——搜索结果复用同一分页条，清除搜索回到目录浏览并保留每页大小等设置；全部状态集中于 `state.pager`，单一入口 `renderFilePage()` / `renderSearchPage()` 渲染，翻页不重新请求接口；**筛选或搜索无结果时分页栏仍保留**（可直接点「全部」切回完整列表，无需刷新），空状态按筛选类型给出明确提示（如「当前目录下没有文件夹」）；**进入子目录自动重置筛选为「全部」**，避免携带上层目录的文件夹/文件筛选导致新目录内容为空
