...
- **文件管理**：按用户浏览文件树、上传、批量上传、新建目录、下载；目录内支持**面包屑导航 + 返回上一级**，新建目录后停留当前目录、可打开或返回；常见文件（图片、音视频、文本/代码、PDF）支持**在线预览**，无需下载即可查看（文本/代码与 PDF 通过浏览器内嵌渲染，`text/html` 强制下载以防 XSS）；下载带实时进度条与速度显示
