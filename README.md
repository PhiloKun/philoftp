...
- **用户管理**：新增/编辑/删除用户，设置主目录、只读/可写、启用状态
- **文件管理**：按用户浏览文件树（面包屑导航）、上传、批量上传、新建目录、下载；文件图标按类型（PDF/图片/压缩包/音视频等）区分，下载带实时进度条
- **状态页**：实时显示 FTP 端口、Web 端口、用户数、被动端口范围、运行时长、Go 版本等
