...
- **概览**：仪表盘式概览页（深空控制台玻璃拟态风格），一次加载完整统计快照：`/api/overview` 提供 4+3 块 KPI 卡片（用户总数/活跃会话/文件目录数/存储使用率 + 管理员数/数据目录/Go 运行时）、用户分布条形图（管理员/普通/启用/禁用/活跃会话）、文件类型分布（按大小降序，隐藏文件归"其它"）、Top 5 大文件、服务器状态（FTP/PASV/Web/FTPS/数据目录）、活跃会话列表（带头像与登录时间）、存储使用率**环形图**（按用量自适应红/黄/青三色）、运行时负载（协程数/Go 版本/健康度）、最近文件更新时间；KPI 卡片可直接跳转对应视图
