...
VERSION_LDFLAGS := -X github.com/philoftp/internal/handler.version=$(VERSION) -X github.com/philoftp/internal/handler.gitCommit=$(GIT_COMMIT) -X github.com/philoftp/cmd/philoftp.version=$(VERSION) -X github.com/philoftp/cmd/philoftp.gitCommit=$(GIT_COMMIT)
...
build:
	go build -ldflags "$(VERSION_LDFLAGS)" -o $(DIST)/macos/$(BINARY)-$(VERSION) ./cmd/philoftp
...
dist-macos:
	GOOS=darwin GOARCH=arm64 go build -ldflags "$(VERSION_LDFLAGS)" -o $(DIST)/macos/$(BINARY)-$(VERSION)-darwin-arm64 ./cmd/philoftp
	GOOS=darwin GOARCH=amd64 go build -ldflags "$(VERSION_LDFLAGS)" -o $(DIST)/macos/$(BINARY)-$(VERSION)-darwin-amd64 ./cmd/philoftp
...
dist-windows:
	CGO_ENABLED=1 GOOS=windows GOARCH=amd64 CC=x86_64-w64-mingw32-gcc go build -ldflags "-H=windowsgui $(VERSION_LDFLAGS)" -o $(DIST)/windows/$(BINARY)-$(VERSION)-windows-amd64.exe ./cmd/philoftp
...
dist-linux:
	GOOS=linux GOARCH=amd64 go build -ldflags "$(VERSION_LDFLAGS)" -o $(DIST)/linux/$(BINARY)-$(VERSION)-linux-amd64 ./cmd/philoftp
